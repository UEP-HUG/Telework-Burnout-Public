...
dataset = "merged_dataset.rds"
# dataset = "merged_dataset_gmh.rds"

merged <- tibble(readRDS(here("data", "Generated datasets", dataset)))
### ### ### ### ### ### ### ### ### ###
...
dat_all <- merged %>%
  ...
  mutate(
    ...
    #
    inc_st_diff_days = as.numeric(time_length(date_soumission.y - date_soumission.x, "years")),
    ...
    pop_source = case_when(
      serocov_work.x == TRUE & work_pilote.x == FALSE & serocov_pop.x == FALSE & pop_pilote.x == FALSE &
    ...
    age_at_st22 = as.numeric(time_length(date_soumission.y - birthdate, "years")),
    # Create age categories reflecting structure in weights dataset
    age_cat_weighting = cut(age_at_st22, c(20, 25, 45, 65, 105), right = FALSE, include.lowest = TRUE),
    # Recode values to match values in weights dataset
    age_cat_weighting = fct_recode(age_cat_weighting, 
                                   "20-24" = "[20,25)",
                                   "25-44" = "[25,45)",
                                   "45-64" = "[45,65)",
                                   "65+" = "[65,105]"),
    ...
    Occupation_label_2 = case_when(str_detect(profession.y, "pas de plusieurs activité") ~ "Recoding impossible",
                                   .default = Occupation_label_2),
    ...
    burnout_interp_dich27 = factor(case_when(
      burnout_score >= 27 ~ "Severe",
      burnout_score<27 ~ "Not Severe", 
      .default = NA),
    ...
    health_general_dich = factor(case_match(health_general,
                                     c("Très bonne", "Bonne") ~ "Good",
                                     c("Moyenne", "Mauvaise", "Très mauvaise") ~ "Not good"),
                                 levels = c("Good", "Not good")
    ),
    
    # mental_state --> dochotomise
    mental_state_dich = factor(case_match(mental_state,
                                   c("Très bon", "Bon") ~ "Good",
    ...
    pandemic_related_work_change = factor(case_when(
      pandemic_change_reason_job_loss_pandemic|
        ...
        pandemic_change_reason_hired |
        new_working_arrangements_schedule_change
      ~ "Change", .default = "No change"),
    ...
    aggression_colleagues = factor(case_when(
      ...
      .default = "No"), 
      levels = c("No", "Yes")),
    # Too much work dichotomized from karasek scale question
    too_much_work = factor(case_when(
      karasek_scale_1_too_much_work %in% c("Tout à fait d’accord", "D’accord") ~ "Yes",
      karasek_scale_1_too_much_work %in% c("Pas d’accord", "Pas du tout d’accord") ~ "No",
      .default = NA),
      levels = c("No", "Yes"))
  )

# Add variables for the dataset that includes the gmh dataset
if(dataset == "merged_dataset_gmh.rds"){
  dat_all <- dat_all %>% 
    mutate(
      # Dichotomize UCLA loneliness score
      UCLA_dichotomized = factor(case_when(UCLA_score >= 6 ~ "Lonely",
                                           UCLA_score < 6 ~ "Not lonely"),
...
dat_all <- dat_all %>% 
  mutate(
    ...
    # French education (for the weighting)
    education_rec_fr = factor(case_when(
      participant_id == "6d6cd138-29b0-11eb-9501-0050568507d9" ~ "Tertiaire",  # brevet avocat
      participant_id == "9e00dd58-29ba-11eb-aa4e-0050568507d9" ~ "Secondaire", # Secrétaire médicale
      participant_id == "d33787cc-3029-11eb-8bf2-0050568507d9" ~ "Secondaire", # Attestation fédérale professionnelle ( ASA)
      participant_id == "8e0e18ce-b966-11ec-8a87-fa163eeab1c6" ~ "Secondaire",  # Ecole privée de secrétariat
      .default = education_rec_fr
    ),
    levels = c("Tertiaire", "Secondaire", "Primaire")
    )
  )
...
dat_all <- dat_all %>% 
  filter(
    ...
    !age_cat %in% c("18-24", "65+"),
    # Filter out "Other" in weighting variables
    sex_en != "Other",
    education_rec_en != "Other",
    # Filter out those who submitted inclusion Q in previous 12 months
    inc_st_diff_days > 1,
    # Filter out 7 NA in overcrowded variable
    !is.na(overcrowded)
    ) %>%        ## Filter out those aged under 25
...
dat <- dat_all %>% 
  ...
  mutate(
    ...
    # Modalities recoding
    new_working_arrangements_activity_reduction_r = if_else(
      is.na(new_working_arrangements_activity_reduction)|new_working_arrangements_no_more_in_place,
      FALSE, new_working_arrangements_activity_reduction),
    new_working_arrangements_activity_increase_r = if_else(
      is.na(new_working_arrangements_activity_increase)|new_working_arrangements_no_more_in_place,
      FALSE,new_working_arrangements_activity_increase),
    
    # Change in working contract, using the new modalities question as well
    percent_change_cat_r = factor(case_when(     
      new_working_arrangements_activity_reduction_r | percent_change < 0 ~ "Reduced",
      new_working_arrangements_activity_increase_r | percent_change > 0 ~ "Increased",
      percent_change == 0 ~ "No change",
      .default = NA),
      levels = c("No change", "Increased", "Reduced")
    ), 
...
# saveRDS(dat_all, here("data", "Generated datasets", paste0(format(Sys.time(), "%Y-%m-%d-%H%M_"),"clean_dataset_incl_salariee.rds"))) # Save the dataset that includes non-salaried people
# saveRDS(dat, here("data", "Generated datasets", paste0(format(Sys.time(), "%Y-%m-%d-%H%M_"),"clean_dataset.rds"))) # Save the cleaned dataset that excludes non-salaried people

if(dataset == "merged_dataset.rds"){
  ## Save the two datasets separately
  ...
  ## Save the two datasets separately
  saveRDS(dat_all, here("data", "Generated datasets", "clean_dataset_incl_salariee_gmh.rds")) # Save the dataset that includes non-salaried people
  saveRDS(dat, here("data", "Generated datasets", "clean_dataset_gmh.rds")) # Save the cleaned dataset that excludes non-salaried people
}
rm(list = ls())
