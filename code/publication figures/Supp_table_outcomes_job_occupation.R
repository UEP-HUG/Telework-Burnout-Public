...
uv_model_final <- uv_model %>% 
  modify_column_hide(columns = c(label, p.value,ci, conf.low, conf.high, estimate, stat_n)) %>% 
...
# Save as docx ####
save_as_docx(
  "Table 2: Participant characteristics according to outcome" = final_table, path = here("output", "publication figures", paste0(format(Sys.time(), "%Y-%m-%d-%H%M_"),"Supp_table_outcomes_job_occupation_", source, ".docx")))
