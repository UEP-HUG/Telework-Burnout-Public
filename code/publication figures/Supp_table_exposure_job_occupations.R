...
dat <- tibble(readRDS(here("data", "Generated datasets", "clean_dataset.rds"))) %>%
  mutate(
    ...
    job_sector = str_wrap(job_sector, width = 30),
    job_sector = fct_relevel(
      fct_infreq(factor(job_sector)),
      "Other", after = Inf),
    job_sector_en = str_wrap(job_sector_en, width = 30),
    job_sector_en = fct_relevel(
      fct_infreq(factor(job_sector_en)),
      "Other", after = Inf),
...
table_telework <- table_telework %>% modify_column_hide(columns = c(n, stat_0))
...
table_combined2 <- table_combined %>% 
  ...
  # hline(i = c(40,43,67), border = fp_border_default(color = "black")) %>% # lines to indicate variable themes
  # bg(part = "all", bg = "grey") %>%
...
# Save as docx ####
save_as_docx(
  "Table 1: Participant summary" = table_combined2, path = here("output", "publication figures", paste0(format(Sys.time(), "%Y-%m-%d-%H%M_"),"Supp_table_exposure_job_occupations_", source, ".docx")))
